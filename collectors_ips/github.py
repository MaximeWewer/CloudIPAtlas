#!/usr/bin/env python3
"""
GitHub IP Sync - Download GitHub IP ranges and generate organized files.

GitHub provides a meta API endpoint with IP ranges for various services:
- hooks: GitHub webhooks
- web: GitHub web interface
- api: GitHub API
- git: Git operations
...
- github_enterprise_importer: Enterprise Importer
...
# Services to extract from GitHub meta API
GITHUB_SERVICES = [
    'hooks',
    'web',
    'api',
                    ...
                    if isinstance(ips, list):
                        ...
                        for ip in ips:
                            if ip and is_valid_ip(ip):
                                all_ips.append(ip)
                                services_ips[service].add(ip)
                        logger.info(f"Service {service}: {len(ips)} ranges")
...
def main():
    ...
    try:
        syncer = GitHubIP(config_path=args.config)
