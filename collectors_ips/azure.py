#!/usr/bin/env python3
"""
Azure IP Sync - Download Azure IP ranges and generate organized files.

This script downloads the latest Azure Service Tags JSON file and generates
...
class AzureIP:
    """Azure IP ranges synchronization manager."""

    def __init__(self, config_path: str = "config.json"):
        """
        Initialize the Azure IP Sync manager.
        ...
        self.url = config['azure']['url']
        self.output_dir = os.path.join("cloud_ips", "azure")
    ...
    def download_data(self) -> dict:
        """
        Download the Azure Service Tags JSON file.
        ...
        try:
            ...
            version = self.data.get('changeNumber', 'N/A')
            logger.info(f"Successfully downloaded (version: {version})")
            return self.data
    ...
    def extract_ips(self) -> tuple:
        ...
        for value in self.data.get("values", []):
            name = value.get("name", "")
            address_prefixes = value.get("properties", {}).get("addressPrefixes", [])

            # Parse name format: "ServiceName" or "ServiceName.Region"
            parts = name.split('.')
            service_name = parts[0]

            # Extract region if present (anything after first dot)
            region_name = None
            if len(parts) > 1:
            ...
            services_ips[service_name].update(address_prefixes)

            # Add to regions if region is present
            if region_name:
                if region_name not in regions_ips:
                    regions_ips[region_name] = set()
                regions_ips[region_name].update(address_prefixes)

            # Add to all IPs
            all_ips.update(address_prefixes)
    ...
    def generate_files(self) -> None:
        ...
        from .ip_utils import calculate_detailed_stats
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download Azure IP ranges and generate organized files"
    )
    parser.add_argument(
        "-c", "--config",
        default="config.json",
    ...
    try:
        syncer = AzureIP(config_path=args.config)
