#!/usr/bin/env python3
"""
Linode IP Sync - Download Linode IP ranges and generate organized files.

Linode provides IP ranges in CSV format.
"""
...
class LinodeIP:
    """Linode IP ranges synchronization manager."""

    def __init__(self, config_path: str = "config.json"):
        """Initialize the Linode IP manager."""
        with open(config_path, 'r', encoding='utf-8') as f:
            config = json.load(f)

        self.url = config['linode']['url']
        self.output_dir = os.path.join("cloud_ips", "linode")
        self.regions_dir = os.path.join(self.output_dir, "regions")

    def download_data(self) -> tuple:
        """
        Download and parse Linode CSV data.
    ...
    def generate_files(self) -> None:
        """Generate all IP range files for Linode."""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Download Linode IP ranges")
    parser.add_argument("-c", "--config", default="config.json")
    args = parser.parse_args()

    try:
        syncer = LinodeIP(config_path=args.config)
