#!/usr/bin/env python3
"""
OpenAI IP Sync - Download OpenAI IP ranges and generate organized files.

OpenAI provides two JSON files:
- chatgpt-user.json: ChatGPT user IP ranges
- gptbot.json: GPTBot crawler IP ranges
"""
...
class OpenAIIP:
    """OpenAI IP ranges synchronization manager."""

    def __init__(self, config_path: str = "config.json"):
        with open(config_path, 'r', encoding='utf-8') as f:
            config = json.load(f)
        self.urls = config['openai']['urls']
        self.output_dir = os.path.join("cloud_ips", "openai")
        self.services_dir = os.path.join(self.output_dir, "services")

    def download_data(self) -> tuple:
        """
        Download OpenAI IP ranges from multiple JSON files.
...
def main():
    ...
    try:
        syncer = OpenAIIP(config_path=args.config)
