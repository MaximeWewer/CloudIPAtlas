#!/usr/bin/env python3
"""
Bingbot IP Sync - Download Bingbot IP ranges and generate organized files.

Bingbot provides a JSON file with IPv4 prefixes used by Bing's crawlers.
"""
...
class BingbotIP:
    """Bingbot IP ranges synchronization manager."""

    def __init__(self, config_path: str = "config.json"):
        with open(config_path, 'r', encoding='utf-8') as f:
            config = json.load(f)
        self.url = config['bingbot']['url']
        self.output_dir = os.path.join("cloud_ips", "bingbot")
    ...
    def extract_ips(self, data: dict) -> list:
        ...
        # Bingbot JSON structure: { "prefixes": [ { "ipv4Prefix" } ] }
        prefixes = data.get('prefixes', [])

        for prefix in prefixes:
            if isinstance(prefix, dict):
                # Bingbot only has ipv4Prefix (no IPv6)
                ip = prefix.get('ipv4Prefix', '')
...
def main():
    ...
    try:
        syncer = BingbotIP(config_path=args.config)
